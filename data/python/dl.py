...
def main():
    try:
        ...
        # 结果统计
        ad_count = len([f for f in os.listdir("./tmp/") if f.startswith("adblock")])
        allow_count = len([f for f in os.listdir("./tmp/") if f.startswith("allow")])
        
        print("\n" + "="*40)
        print(f"✓ 任务完成！共下载: {ad_count}个拦截规则, {allow_count}个白名单规则")
        print(f"文件保存在: {os.path.abspath('./tmp/')}")
